...
from src.runtime.temporal import TemporalHistoryBuffer, ScaleLatentState
...
class WalkForwardEngine:
    ...
    def run_backtest(self):
        ...
        # 1. Carrega dados
        data = self.loader.load_multi_timeframe_sol(train_ratio=1.0, max_samples=self.max_samples)
        data_all = {}
        ...
        # 2. Instancia modelos
        n_features = data_all["15m"][0].shape[1]  # todas TFs têm 28 features
        predictors: Dict[str, HSAMAOnlineRuntime] = {}
        for tf in self.TIMEFRAMES:
            pred_model = HSAMA(
                in_features=data_all[tf][0].shape[1],
                out_features=2,
                num_nodes=8,
                state_dim=16,
                max_hops=2,
        ...
        for w in range(N_WALKS):
            ...
            for tf in self.TIMEFRAMES:
                ...
                rt.global_step = 0
                # Reset do contexto GRU cacheado para não vazar entre walks
                for name in rt.multi_scale_builder.scale_order:
                    rt.multi_scale_builder.scale_states[name] = ScaleLatentState()
            trader.history_buffer = TemporalHistoryBuffer(max_events=trader.multi_scale_builder.required_history())
            trader.global_step = 0
            for name in trader.multi_scale_builder.scale_order:
                trader.multi_scale_builder.scale_states[name] = ScaleLatentState()
            ...
            tmp_ckpt_path = f"models/monolithic_temp_pre_oos_walk_{w}.pt"
            torch.save({
            ...
            print(f"  [Walk {w+1}] OOS Net PnL: {net_stream.sum():+.4f} | B&H: {ret_np.sum():+.4f}")

            wf_net_stream.append(net_stream)
            wf_pos_np.append(pos_np)
            wf_gross_np.append(gross_np)
            wf_cost_np.append(cost_np)
            wf_ret_np.append(ret_np)

        # --- FIM DOS WALKS: Concatena resultados ---
        ...
        print(f"\n{'='*65}")
        print(f" [Resultado Walk-Forward Combinado] ({N_WALKS} walks)")
        print(f"{'='*65}")
        ...
        for step in range(test_len):
            ...
            cumul  += net_val
            records.append({
                "step": step, "position": round(float(pos_np[step]), 6),
                ...
                "net_pnl": round(net_val, 6), "cumul_net_pnl": round(cumul, 6),
            })
        df = pd.DataFrame(records)
        csv_path = "artifacts/SOL_walkforward_trades_oos.csv"
        ...
        axes[0].plot(agent_equity, label="Walk-Forward Agent", color="#00c4ff", linewidth=2)
        # Linhas verticais separando os walks
        cumul_len = 0
        for wi in range(N_WALKS - 1):
            cumul_len += WALK_TEST_LEN
            axes[0].axvline(cumul_len, color="yellow", linewidth=1, linestyle="--", alpha=0.5)
            axes[1].axvline(cumul_len, color="yellow", linewidth=1, linestyle="--", alpha=0.5)
        axes[0].set_title(f"SOL/USDT -- Walk-Forward OOS ({N_WALKS} walks)")
        ...
        plt.tight_layout()
        plot_path = "artifacts/plots/SOL_walkforward_equity.png"
        ...
        ckpt_path = f"models/walkforward_{ckpt_ts}.pt"
        checkpoint = {
            "timestamp": ckpt_ts, "max_samples": self.max_samples, "test_len": int(test_len),
            "hyperparams": {
                "PREDICTOR_WARMUP_EPOCHS": PREDICTOR_WARMUP_EPOCHS,
                "JOINT_EPOCHS": JOINT_EPOCHS, "TRADING_COST_BPS": TRADING_COST_BPS,
                "TRADER_POSITION_DEADZONE": TRADER_POSITION_DEADZONE,
                "WALK_TRAIN_LEN": WALK_TRAIN_LEN, "WALK_TEST_LEN": WALK_TEST_LEN, "N_WALKS": N_WALKS,
            },
            "predictors": {
                tf: {
            ...
            "sig_normalizer": {
                "mu": sig_normalizer.mu, "var": sig_normalizer.var,
                "decay": sig_normalizer.decay, "initialized": sig_normalizer._initialized,
            },
            "oos_metrics": {
                "agent_net_return": float(agent_equity[-1]), "bh_return": float(bh_equity[-1]),
                "sharpe": float(sharpe), "max_dd_pct": float(max_dd), "n_trades": int(n_trades),
                "avg_holding": float(avg_holding), "long_pct": float(long_pct),
                "short_pct": float(short_pct), "flat_pct": float(flat_pct),
